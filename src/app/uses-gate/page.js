...
export default function GateUsagePage() {
  const [usage, setUsage] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);
  const [editingUsage, setEditingUsage] = useState(null);
  const [formData, setFormData] = useState({ flight_id: '', route_id: '', leg_sequence_no: '', airport_id: '', gate_no: '', usage_type: 'Arrival' });

  const columns = [
    { key: 'flight_id', label: 'Flight' },
    { key: 'gate_no', label: 'Gate' },
  ...
  const fetchUsage = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/uses-gate');
  ...
  const handleEdit = (u) => {
    setEditingUsage(u);
    setFormData({ flight_id: u.flight_id, route_id: u.route_id, leg_sequence_no: u.leg_sequence_no, airport_id: u.airport_id, gate_no: u.gate_no, usage_type: u.usage_type });
  };

  
  const handleDelete = async (row) => {
    try {
      const res = await fetch(`/api/uses-gate?flight_id=${row.flight_id}&route_id=${row.route_id}&leg_sequence_no=${row.leg_sequence_no}&airport_id=${row.airport_id}&gate_no=${row.gate_no}`, { method: 'DELETE' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ flight_id: '', route_id: '', leg_sequence_no: '', airport_id: '', gate_no: '', usage_type: 'Arrival' });
    ...
    <div className="animate-fade">
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        ...
        <form onSubmit={handleSubmit} style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
          ...
          <div className="form-group"><label>Gate No</label><input type="number" name="gate_no" value={formData.gate_no} onChange={handleInputChange} required disabled={!!editingUsage} /></div>
          <div className="form-group"><label>Usage Type</label><select name="usage_type" value={formData.usage_type} onChange={handleInputChange}><option value="Arrival">Arrival</option><option value="Departure">Departure</option></select></div>
          <div style={{ gridColumn: 'span 3', display: 'flex', gap: '1rem' }}>
            <button type="submit">{editingUsage ? 'Update' : 'Log Usage'}</button>
            {editingUsage && <button type="button" className="btn-secondary" onClick={() => { setEditingUsage(null); setFormData({ flight_id: '', route_id: '', leg_sequence_no: '', airport_id: '', gate_no: '', usage_type: 'Arrival' }); }}>Cancel</button>}
          </div>
