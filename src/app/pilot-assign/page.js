...
export default function PilotAssignPage() {
  const [assignments, setAssignments] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);
  const [formData, setFormData] = useState({ flight_id: '', route_id: '', leg_sequence_no: '', pilot_id: '' });
  ...
  const handleDelete = async (row) => {
    try {
      const res = await fetch(`/api/pilot-assign?flight_id=${row.flight_id}&route_id=${row.route_id}&leg_sequence_no=${row.leg_sequence_no}&pilot_id=${row.pilot_id}`, { method: 'DELETE' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ flight_id: '', route_id: '', leg_sequence_no: '', pilot_id: '' });
      fetchAssignments();
  ...
  return (
    <div className="animate-fade">
      <h1>Pilot Assignments</h1>
      <div className="card" style={{ marginBottom: '2rem' }}>
        <h2>Assign Pilot to Leg</h2>
        <form onSubmit={handleSubmit} style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1rem' }}>
          ...
          <div className="form-group"><label>Pilot ID</label><input type="number" name="pilot_id" value={formData.pilot_id} onChange={handleInputChange} required /></div>
